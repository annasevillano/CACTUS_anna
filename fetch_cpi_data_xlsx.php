...

{
    ...
    $xls_file_name = $_FILES['cpi_data_xlsx_file']['name'];
    $xls_file_size = $_FILES['cpi_data_xlsx_file']['size'];
    $xls_file_tmp = $_FILES['cpi_data_xlsx_file']['tmp_name'];
    $xls_file_type = $_FILES['cpi_data_xlsx_file']['type'];
    $xls_file_error = $_FILES['cpi_data_xlsx_file']['error'];
    ...
    clear_table($db, $cpi_table);

    if (!$_FILES['cpi_data_xlsx_file']['error']) {
        if ($xls_file_name != "") {
            ...
            foreach ($objPHPExcelModules->getWorksheetIterator() as $worksheet) {
                ...
                for ($i = 0; $i < $highestRow; $i++) {
                    ...
                    for($j = 0 ; $j < $num_years; $j++) {
                        ...
                        $myString = $worksheet->getCellByColumnAndRow($col, $row)->getValue();
                        if (trim($myString) != "") {
                            $cpi_value = floatval($myString);
                            $year = $years[$j];
                            // Push to db
                            $query = "INSERT INTO $cpi_table SET
                                                country='$country',
            ...
            // Emptry the table and fill with new data
            clear_table($db, $cpi_desc_table);

            $hundred_year = 2010;
            $year_begin = $years[0];
            $year_end = $years[$num_years-1];
            $query = "INSERT INTO $cpi_desc_table SET
